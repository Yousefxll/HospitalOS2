...
export async function verifySubscriptionEnforcement(
  ...
  if (user.role === 'syra-owner') {
    return {
      passed: true,
      message: 'SYRA Owner - subscription check skipped',
    };
  }
...
export async function verifyOwnerSeparation(
  ...
  // Only check for syra-owner
  if (user.role !== 'syra-owner') {
    return {
      passed: true,
      message: 'Not SYRA Owner - check skipped',
    };
  }
...
export async function runQualityGate(
  ...
  try {
    ...
    console.error('[runQualityGate] Route scan failed:', error);
    results.push({
      passed: false,
  ...
  try {
    ...
    console.error('[runQualityGate] Cross-tenant tests failed:', error);
    results.push({
      passed: false,
