...
export async function policyEngineSearch(req: Request, tenantId: string, body: Record<string, any>) {
  const { orgProfile, contextRules } = await getOrgContextSnapshot(req, tenantId);
  const url = buildGatewayUrl(req, '/api/sam/policy-engine/search');
...
export async function policyEngineListPolicies(
  req: Request,
  tenantId: string,
  queryParams?: GatewayQuery
) {
  const { orgProfile, contextRules } = await getOrgContextSnapshot(req, tenantId);
  const url = buildGatewayUrl(req, '/api/sam/policy-engine/policies', queryParams);
...
export async function policyEngineGetFile(req: Request, tenantId: string, policyId: string) {
  const { orgProfile, contextRules } = await getOrgContextSnapshot(req, tenantId);
  const url = buildGatewayUrl(req, `/api/sam/policy-engine/policies/${policyId}/file`);
...
export async function policyEngineDeletePolicy(req: Request, tenantId: string, policyId: string) {
  const { orgProfile, contextRules } = await getOrgContextSnapshot(req, tenantId);
  const url = buildGatewayUrl(req, `/api/sam/policy-engine/policies/${policyId}`);
  const headers = new Headers({ 'x-tenant-id': tenantId });
  attachContextHeaders(headers, orgProfile, contextRules);
  const response = await forwardGatewayRequest(req, url, {
    method: 'DELETE',
...
export async function policyEngineReprocessPolicy(
  req: Request,
  tenantId: string,
  policyId: string,
  mode?: string
  ...
  const { orgProfile, contextRules } = await getOrgContextSnapshot(req, tenantId);
  const url = buildGatewayUrl(req, `/api/sam/policy-engine/policies/${policyId}/reprocess`);
  const response = await forwardGatewayRequest(req, url, {
    method: 'POST',
...
export async function policyEngineRewritePolicy(
  req: Request,
  tenantId: string,
  policyId: string,
  ...
  const { orgProfile, contextRules } = await getOrgContextSnapshot(req, tenantId);
  const url = buildGatewayUrl(req, `/api/sam/policy-engine/policies/${policyId}/rewrite`);
  const response = await forwardGatewayRequest(req, url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-tenant-id': tenantId },
