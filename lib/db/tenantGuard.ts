 ...
 * - ALL writes/reads must go to tenant DB only: syra_tenant__<tenantId>
...
export function assertTenantDatabase(dbName: string): void {
  if (!dbName.startsWith('syra_tenant__')) {
    throw new Error(
      ...
      `All operations must use tenant databases (syra_tenant__<tenantId>). ` +
...
export function shouldBlockCollectionAccess(collectionName: string, dbName?: string): boolean {
  ...
  if (dbName && !dbName.startsWith('syra_tenant__')) {
    return true;
  }
