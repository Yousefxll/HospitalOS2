 ...
 * 
 * Unified naming pattern: syra_tenant__<tenantKey>
 * 
 * MongoDB Atlas has a maximum database name length of 64 bytes.
 * This function uses the unified pattern: syra_tenant__<tenantKey>
 * 
 * @param tenantId - Tenant identifier
 * @returns Database name: syra_tenant__<tenantId>
 */
export function generateTenantDbName(tenantId: string): string {
  const PREFIX = 'syra_tenant__'; // 13 chars
