...
export async function translateToEnglish(
  ...
  if (provider === 'openai') {
    try {
      ...
      if (!openai) {
        console.warn('OpenAI client not available, using fallback translation');
        return trimmedText;
      }
      ...
      console.warn('Unexpected OpenAI response format, using fallback');
      return trimmedText;
    } catch (error: any) {
  ...
  console.warn(`Unknown translation provider: ${provider}, using fallback`);
  return trimmedText;
}
