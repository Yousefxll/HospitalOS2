...
export const CORS_CONFIG = {
  ...
  ALLOWED_ORIGINS: process.env.CORS_ALLOWED_ORIGINS?.split(',').map(o => o.trim()) || 
...
export const SECURITY_HEADERS = {
  ...
  CSP: [
    ...
    "font-src 'self' data:",
    "connect-src 'self'",
    env.OPENAI_API_KEY ? `connect-src 'self' https://api.openai.com` : '',
    env.POLICY_ENGINE_URL ? `connect-src 'self' ${env.POLICY_ENGINE_URL}` : '',
