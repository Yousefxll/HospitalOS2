...
export async function requirePermission(
  ...
  if (auth instanceof NextResponse) {
    if (process.env.DEBUG_AUTH === '1') {
      console.error(`[requirePermission] Auth failed - status: ${auth.status}`);
    }
  ...
  if (!hasPermission && !isAdmin) {
    if (process.env.DEBUG_AUTH === '1') {
      console.error(`[requirePermission] Permission check failed - userId: ${auth.userId}, required: ${permissionsArray.join(' or ')}, user has: ${userPermissions.join(', ')}`);
    }
  ...
  if (process.env.DEBUG_AUTH === '1') {
    console.log(`[requirePermission] Permission check passed - userId: ${auth.userId}, permission: ${permissionsArray.join(' or ')}`);
  }
...
export async function requireRoutePermission(
  ...
  if (!requiredPermission) {
    ...
    if (process.env.DEBUG_AUTH === '1') {
      console.error(`[requireRoutePermission] Route not in permission map: ${route}`);
    }
    return NextResponse.json(
      { 
        error: 'Forbidden', 
