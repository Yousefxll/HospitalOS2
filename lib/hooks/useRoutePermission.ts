...
export function useRoutePermission(route: string) {
  ...
  useEffect(() => {
    if (meLoading || !me || platformLoading) {
      setIsLoading(meLoading || platformLoading);
      return;
    }
    ...
    if (!hasAccess) {
      ...
      const samRoutes = ['/policies', '/ai', '/sam'];
  ...
  }, [me, meLoading, platformData, platformLoading, route, router]);
