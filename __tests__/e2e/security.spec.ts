...
async function login(page: Page, email: string, password: string, tenantId?: string) {
  ...
  // Wait for API response and step 2 to appear
  // Check for either password field (step 2) or error message
  await page.waitForFunction(
    () => {
      const passwordInput = document.querySelector('input[id="password"]');
      const tenantSelect = document.querySelector('select[id="tenant"]');
      const errorAlert = document.querySelector('[role="alert"]');
      return passwordInput || tenantSelect || (errorAlert && errorAlert.textContent?.includes('Invalid'));
    },
    { timeout: 15000 }
  );
  ...
  if (tenantId) {
    // Wait for tenant select to be visible and enabled, if it exists
    const tenantSelect = await page.$('select[id="tenant"]');
    if (tenantSelect) {
      await page.selectOption('select[id="tenant"]', tenantId);
    }
  }
...
test('Subscription expired blocks login', async ({ page }) => {
  ...
  // Step 1: Identify
  await page.fill('input[id="email"]', EXPIRED_TENANT_USER.email);
  await page.click('button[type="submit"]');
  
  // Wait for step 2 to appear (password field or tenant select)
  // Give it enough time for the API call to complete
  try {
    await page.waitForSelector('input[id="password"], select[id="tenant"]', { timeout: 20000, state: 'visible' });
  } catch (error) {
    ...
    const emailInput = await page.$('input[id="email"]:not([disabled])');
    if (emailInput) {
      // Still on identify step - check for error
      const errorText = await page.textContent('[role="alert"]') || await page.textContent('body');
      // If there's an error, that's OK - subscription might block at identify
      if (errorText?.toLowerCase().includes('subscription') || 
          errorText?.toLowerCase().includes('expired') ||
          errorText?.toLowerCase().includes('blocked')) {
        return; // Test passes
  ...
  await page.fill('input[id="password"]', EXPIRED_TENANT_USER.password);
  
  // Select tenant if needed
  if (EXPIRED_TENANT_USER.tenantId) {
    const tenantSelect = await page.$('select[id="tenant"]');
    if (tenantSelect) {
      await page.selectOption('select[id="tenant"]', EXPIRED_TENANT_USER.tenantId);
    }
  }
...
test('Subscription expired blocks /api/auth/me', async ({ page }) => {
  ...
  // Try to access /api/auth/me with expired tenant
  const response = await page.request.get(`${BASE_URL}/api/auth/me`, {
    headers: {
      'Cookie': `auth-token=expired-token`, // Would need actual expired token
    },
  });
...
test('Platform not visible if user not entitled', async ({ page }) => {
  ...
  if (currentUrl.includes('/platforms/syra-health')) {
  ...
  if (currentUrl.includes('/platforms') && !currentUrl.includes('/platforms/')) {
    ...
    try {
      await Promise.race([
        ...
        page.waitForURL(url => url.includes('/platforms/') || url.includes('/welcome'), { timeout: 5000 }),
        // Option 2: Tiles appear (user has multiple platforms)
        page.waitForSelector('[data-testid^="platform-tile-"], [data-platform]', { timeout: 5000 })
      ]);
      ...
      if (updatedUrl.includes('/platforms/syra-health')) {
      ...
      if (finalUrl.includes('/platforms/syra-health') || finalUrl.includes('/welcome')) {
      ...
      // Still on /platforms with no tiles - this is unexpected, but redirect should happen
  ...
  throw new Error(`Unexpected URL state: ${currentUrl}. Expected /platforms, /platforms/syra-health, or /welcome`);
});
