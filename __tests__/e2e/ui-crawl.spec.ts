...
async function login(page: Page, email: string, password: string, tenantId?: string) {
  ...
  // Handle tenant selector (optional - only appears if user has multiple tenants)
  // Radix UI Select uses SelectTrigger with id="tenant", not a native select element
  if (tenantId) {
    // Try multiple selector strategies
    const tenantSelectors = [
      'select#tenant',           // Native select (if used)
      'select[name="tenant"]',   // Alternative native select
      '#tenant',                 // Radix UI SelectTrigger (has id="tenant")
      '[data-testid="tenant-select"]', // Alternative test ID
    ];
    
    let tenantSelected = false;
    for (const selector of tenantSelectors) {
      try {
        const tenantElement = page.locator(selector);
        const count = await tenantElement.count();
        
        if (count > 0) {
          const isVisible = await tenantElement.isVisible({ timeout: 2000 }).catch(() => false);
          
          if (isVisible) {
            // Check if it's a native select or Radix UI Select
            const tagName = await tenantElement.evaluate((el) => el.tagName.toLowerCase()).catch(() => '');
            
            if (tagName === 'select') {
              // Native select element
              await tenantElement.selectOption(tenantId);
              tenantSelected = true;
              break;
            } else {
              // Radix UI Select - click trigger, then select option
              await tenantElement.click({ timeout: 2000 }).catch(() => {});
              // Wait for dropdown to open
              await page.waitForTimeout(300);
              // Click the option with matching value
              await page.click(`[role="option"][data-value="${tenantId}"]`, { timeout: 2000 }).catch(() => {});
              tenantSelected = true;
              break;
            }
          }
    ...
    // If tenant selector not found/visible, that's OK - user might have only one tenant (auto-selected)
    if (!tenantSelected) {
      console.log(`[login] Tenant selector not found/visible for ${email} - assuming single tenant (auto-selected)`);
    }
  }
...
async function setPlatform(page: Page, platform: 'sam' | 'health') {
  const response = await page.request.post(`${BASE_URL}/api/platform/switch`, {
    data: { platform },
