...
describe('getAccessibleModules', () => {
  ...
  it('should return only permitted modules for regular user', () => {
    const permissions = ['policies.view', 'account.view'];
    const modules = getAccessibleModules(permissions);
    
    // Should include policies and account
    const moduleIds = modules.map(m => m.id);
    expect(moduleIds).toContain('policies');
  ...
  it('should filter modules correctly based on required permissions', () => {
    const permissions = ['opd.dashboard.view', 'policies.view'];
    const modules = getAccessibleModules(permissions);
    
    const moduleIds = modules.map(m => m.id);
    expect(moduleIds).toContain('opd-dashboard');
    expect(moduleIds).toContain('policies');
