...
export async function POST(
  ...
  return withAuthTenant(async (req, { user, tenantId }) => {
    try {
      ...
      const validation = moveNodeSchema.safeParse(body);
      ...
      // Pass the request so moveOrgNode can use it for getTenantDbFromRequest
      const result = await moveOrgNode(req, nodeId, validation.data.newParentId);
      ...
      console.error('[structure/org/[nodeId]/move] POST error:', error);
      return NextResponse.json(
