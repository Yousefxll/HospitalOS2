...
// Note: We need to use FormData compatible API for policy-engine ingest
// Since we're in Node.js, we'll use node-fetch or native fetch with Blob
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        // Upload to policy-engine service via Next.js proxy (async, non-blocking)
        // This ensures the policy appears in the Library page
        // Note: policy-engine will create its own policyId (different from MongoDB policyId)
        // This is acceptable - MongoDB stores metadata/tags, policy-engine processes files for Library
        setTimeout(async () => {
          try {
            // Create FormData - in Node.js 18+, FormData supports Blob directly
            ...
            if (ingestResponse.ok) {
              ...
              console.log(`✅ Policy ${file.name} ingested into policy-engine, jobs: ${ingestData.jobs?.length || 0}`);
              // Note: policy-engine creates its own policyId - this is expected and acceptable
            } else {
              const errorText = await ingestResponse.text().catch(() => 'Unknown error');
              console.warn(`⚠️ Failed to ingest policy ${file.name} into policy-engine: ${ingestResponse.status} - ${errorText}`);
              // Don't fail - MongoDB record is still created, policy just won't appear in Library until policy-engine is available
            }
          } catch (ingestError) {
            console.error(`❌ Error ingesting policy ${file.name} into policy-engine:`, ingestError);
            // Don't fail - MongoDB record is still created
          }
