...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!searchResponse.ok) {
      ...
      console.error('SYRA search error:', errorText);
      return NextResponse.json(
...
---`;
    ...
    // Step 5: Get policy metadata from policy-engine with tenantId as a query parameter
    const policiesUrl = `${env.POLICY_ENGINE_URL}/v1/policies?tenantId=${tenantId}`;
