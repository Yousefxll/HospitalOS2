...
export const POST = withAuthTenant(async (req: NextRequest, { tenantId, userId, user }) => {
  ...
  return withIdempotency({
    ...
    clientRequestId,
    handler: async () => {
      const roomsCol = admin.db.collection(CLINICAL_INFRA_COLLECTIONS.rooms);

      const existing = await roomsCol
        .find({ tenantId, unitId, name: { $in: unique } }, { projection: { _id: 0, id: 1, name: 1 } })
        .toArray();
      const existingByName = new Map(existing.map((e: any) => [String(e.name || '').toLowerCase(), e]));
      ...
      for (const name of unique) {
        ...
        try {
          // Insert (uniqueness should be enforced by index on tenantId+unitId+name)
          await roomsCol.insertOne(doc);
          await finishAudit({ db: admin.db, tenantId, auditId, ok: true });
          created.push(doc);
        } catch (e: any) {
          const msg = String(e?.message || e);
          // Duplicate key => treat as graceful no-op
          if (msg.toLowerCase().includes('duplicate') || e?.code === 11000) {
            await finishAudit({ db: admin.db, tenantId, auditId, ok: false, error: 'DUPLICATE' });
            skipped.push({ name, reason: 'DUPLICATE' });
          } else {
            await finishAudit({ db: admin.db, tenantId, auditId, ok: false, error: msg });
