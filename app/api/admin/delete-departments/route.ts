...
export const DELETE = withAuthTenant(async (req, { user, tenantId }) => {
  try {
    console.log(`[delete-departments] 🚀 Starting deletion for tenant: ${tenantId}`);
    ...
    console.log(`[delete-departments] Found ${nodesResult.length} total org nodes`);
    ...
    console.log(`[delete-departments] Found ${departmentsToDelete.length} departments to delete:`);
    ...
    for (const dept of departmentsToDelete) {
      try {
        console.log(`[delete-departments] 🗑️  Deleting department: ${dept.name} (id: ${dept.id})`);
        ...
        if (result instanceof NextResponse) {
          ...
          console.error(`[delete-departments] ❌ Failed to delete ${dept.name}: ${errorText}`);
          errors.push({
            name: dept.name,
            id: dept.id,
            error: errorText,
          ...
          console.log(`[delete-departments] ✅ Successfully deleted: ${dept.name}`);
          deleted.push(dept.name);
        }
      } catch (error: any) {
        console.error(`[delete-departments] ❌ Exception deleting ${dept.name}:`, error);
        errors.push({
          name: dept.name,
          id: dept.id,
