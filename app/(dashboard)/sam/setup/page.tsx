...
export default function SamSetupPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!maturityStage) {
        throw new Error('Maturity stage is required.');
      }
      ...
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}));
        throw new Error(payload.error || 'Failed to save organization profile');
      }

      toast({
        title: 'Organization context saved',
        description: 'SAM will use this context for analysis and recommendations.',
      });
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4">
      <Card>
        <CardHeader>
          <CardTitle>Set up your organization context</CardTitle>
          <CardDescription>
            SAM uses this profile to tailor analysis, prioritize gaps, and recommend next steps.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="text-sm text-muted-foreground">Loading organization profile…</div>
          ) : (
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-between rounded-md border p-3">
                <div className="space-y-1">
                  <p className="text-sm font-medium">Part of a group?</p>
                  <p className="text-xs text-muted-foreground">
                    Turn on if policies or standards are shared across a parent organization.
                  </p>
                </div>
              ...
              {isPartOfGroup && (
                <div className="space-y-2">
                  <label className="text-sm font-medium">Group identifier</label>
                  <Input
                    value={groupId}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Standards & frameworks</label>
                <Input
                  value={standardsText}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Add multiple entries separated by commas.
                </p>
              </div>
