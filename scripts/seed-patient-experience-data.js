 ...
 * 
 * Run with: node scripts/seed-patient-experience-data.js
 */
...
const DB_NAME = process.env.DB_NAME || process.env.MONGODB_DB || 'hospital_ops';
...
async function seedData() {
  ...
  try {
    ...
    const resolvedVisits = visits.filter(v => {
      // Check if it's a complaint (not praise) and resolved
      const isComplaint = !PRAISE_CATEGORIES.some(p => p.key === v.domainKey);
