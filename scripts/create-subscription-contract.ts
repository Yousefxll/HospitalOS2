...
async function createSubscriptionContract(tenantId: string) {
  ...
  try {
    ...
    if (existingContract) {
      ...
      console.log(`   Platforms: SAM=${existingContract.enabledPlatforms.sam}, Health=${existingContract.enabledPlatforms.syraHealth}`);
    ...
    console.log(`     - SYRA Health: ${contract.enabledPlatforms.syraHealth ? '✅' : '❌'}`);
