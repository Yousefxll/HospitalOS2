 ...
 * 
 * FAILS if it finds:
 * A) Tenant-scoped routes using getPlatformDb() or db("syra_platform")
...
const VIOLATION_PATTERNS = [
  {
    name: 'A) Tenant route using getPlatformDb or sira_platform',
    pattern: /(getPlatformDb\(\)|db\(['"]syra_platform['"]\))/,
